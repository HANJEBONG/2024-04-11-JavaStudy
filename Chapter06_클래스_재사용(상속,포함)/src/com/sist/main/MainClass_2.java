 ...
 *         = 정적 변수 : 컴파일러에 의해서 자동으로 저장 => 메모리 공간 한개 (모든 클래스에서 공유)
 *           1) 자동으로 기본 초기화
 ...
 *         = 지역 변수 : 메소드 안에서만 사용이 가능한 변수
 *                    => 지역변수 , 매개변수
 *                    => 루프변수 , 임시변수
 *                    ---------------------------- 메소드가 종료가 되면 자동으로 사라지는 변수
 *                    *** 변수의 초기화가 없다 (반드시 초기화후에 사용)
 *         = 객체 생성시 호출해야되는 메소드 : 생성자
 *           생성자는 필요시에 사용한다
 *           ** 멤벼변수 , 정적변수 , 메소드 , 생성자 => 필요한 것만 골라서 사용이 가능
 *              --------------------------- 구성요소는 다 필요한 것이 아니다
 *              => 변수
 *              => 메소드
 *              => 변수 + 메소드
 ...
 *           --------------------------- 화면 UI , 웹 (데이터베이스 연결 , 자동 로그인 , 쿠키읽기 , 권한 부여)
 *           생성자
 *           ----
 *             여러개 사용이 가능 , 없는 경우 (자동으로 컴파일러가 첨부=매개변수가 없는 생성자:default 생성자)
 *             => 상속의 예외 조건
 *             => 클래스명과 동일
 *             => 리턴형이 없다
 *             => 객체 생성시에 호출되는 메소드
 ...
 *             => 여러개가 있는 경우 => 오버로딩
 *                오버로딩 : 중복 함수 정의 => 같은 이름의 메소드를 여러개 생성
 *                => 메소드명이 동일
 *                => 매개변수의 갯수나 데이터형이 다르다
 *                => 리턴형은 관계없다
 *                => 접근지정어는 관계없다
 ...
 *                       return ...
 *                    }
 *                    *** 메소드는 호출시마다 처음부터 실행해서 메소드가 종료가 되면 원래 호출한 위치로 돌아온다
 *                               ----------------    --------------------------
 *                    *** 메소드 사용 이유
 *                    => 반복 제거 (반복적인 코딩)
 *                               ---------- 반복문 가능 => 메소드 => 코드의 간소화
