 ...
 *         사용 => 객체 => new로부터 생성
 *               --- 사물을 단순화
 *                관련된 데이터와 메소드를 관리하는 메모리 영역 (변수 => 뎅이터값을 관리)
 *                ---------------------------------
 *                define은 중요하지 않다
 *                => 권위가 있는 사람이 이름 부여
 *                => 예)
 *                       하둡 => 개발자의 아들의 장난감 이름 (노란 코끼리)
 *                       클라우드 (구름) => 구글사장 => 네트워크
 *         class className{
 *             클래스영역에서는 제어문 , 연산처리 , 값을 다시 대입 => 불가능
 *             ------- 선언 , 선언과 동시에 값 대입
 *                     ---------------------
 *                     int a;
 *                     int a=100;
 ...
 *               = static 변수 ==> 공통변수 ==> Method Area => 메모리가 한개만 생성
 *                 --------- 컴파일러에 의해서 생성
 *                 --------- 접근시에는 클래스명.변수명
 *             ---------------------------------
 *              메소드 : 변수 활용 => 사용자가 행위 ...
