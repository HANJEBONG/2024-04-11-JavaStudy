 ...
 *                               jdbc:oracel:thin:@localhost:1521:XE
 *                               			 ---- 연결만 해주는 역할 => 속도가 늦다
 *      3) SQL문장을 오라클 전송
 *          => SQL문장을 만든다 => String
 ...
 *      4) SQL문장 실행후에 결과값 받기
 *         --------------------- SELECT
 *            INSERT/UPDATE/DELETE => executerUpdate() : commit() 호출하고 있다 => 데이터 갱신
 *            SELECT => executeQuery() => 실행된 결과값을 읽어온다
 *            
 *            ResultSet rs=ps.execuerQuery()
 *                 SELECT id,name,sex
 *            ---------------------------------
 *            컬럼명    ID   NAME   SEX
 *            ---------------------------------
 ...
 *         conn.close()
 *         -------------------------------
 *         1. Connection => 반드시 닫기수행 (닫기가 안되면 Connection은 사용하지 않는데 오라클 연결...)
 *         							   	------------------------------------------------> DDOS
 *         2. 사용자가 Connection 을 한개만 사용할 수 있게 만든다 : 싱글턴
 *         
 *         3. JDBC => 웹(DBCP) => 라이브러리 등장 (MyBatis / Hibernate)
 *         									  | 실무      | 자동으로 SQL문장을 만들어주는(JPA)
 *             		 	| Connection을 미리 연결
 *            | 연결시마다 => Connection 생성
 *      => 주의점 : 문자열로 되어 있다
 *      		  ----------------> 컴파일시 에러가 없다 => 실행시 에러 발생
...
public class GoodsDAO {
	...
	public ArrayList<GoodsVO> goodsListData(int page){
		...
		try {
			...
			// 오라클에서 페이지 나누기 => 인라인뷰 => 가상 칼럼 : rownum (자르기)
			// rownum은 Top-N => 처음부터 몇개 => 중간을 자르는 것은 불가능
			// ? 에 값 채우기
			int rowSize=12;
