...
public class GoodsDetailPanel extends JPanel implements ActionListener, ItemListener{
	...
	public GoodsDetailPanel(ControlPanel cp) {
		...
		la4=new JLabel();
		la4.setBounds(470,125,60,45);
		la5=new JLabel();
		la5.setBounds(540,110,60,45);
		la6=new JLabel();
		...
		add(la2);add(la3);add(la4);add(la5);add(la6);add(la7);add(la8);
		b1=new JButton("장바구니");
		...
		JPanel p=new JPanel();
		p.add(b1);p.add(b2);
		p.setBounds(470,350,350,30);
		add(p);
		pLa=new JLabel("가격");
		pLa.setBounds(470,270,200,30);
		add(pLa);
		tLa=new JLabel("총 구매액");
	...
	public void print(int no) {
		...
		la2.setText("<html><font size=12><b>"+vo.getGoods_name()+"</b></font></html>");
		...
		tLa.setText("총 구매 가격:"+vo.getGoods_price());
	}
	@Override
