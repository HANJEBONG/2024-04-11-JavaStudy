 ...
 *    java.lang : 자바의 기본적으로 사용이 많은 클래스 모아서 관리 => import 를 생략할 수 있다
 *    ---------- 292 page  
 *      = Object : 자바의 모든 클래스(사용자 정의)의 상위 클래스 => 최상위 클래스
 *                 => 모든 클래스는 Object 로부터 상속을 받는다
 *                 => 생략이 가능 => extends Object
 *                 => 데이터형중에 가장 큰 데이터형
 *                    --------------------- 모든 데이터를 받아서 저장이 가능
 *                 => 라이브러리에 메소드의 리턴형은 대부분이 Object => 객체 형변환
 *                                                          --------
 ...
 *                      
 *                      A a=new A();
 *                      System.out.println(a) => toString() 을 생략할 수 있다 => 묵시적
 ...
 *                   4. finalize() : 소멸자 => 객체 메모리 해체 => 자동 호출
 *                                   => 호출이 안되면 메모리에 남아 있다..
 *                                   => System.gc() : 사용하지 않는 메모리를 회수 요청
 *                                   => 웹/애플리케이션에서는 잘 사용하지 않는다 => 멀티미디어 (동영상,화상채팅,CCTV)
 *                   ***5. equals() : 객체를 비교할때
 *                                 => 메모리 주소값 (디폴트)
 *                                 => 멤버변수값을 비교 (오버라이딩)
 *                   
...
class Student{
	...
	// hashCode 변경 => 라이브러리는 필요한 경우에 대부분 오버라이딩을 해서 사용
	@Override
	public int hashCode() {
		// TODO Auto-generated method stub
 ...
 *           -------------------------
 *             상수 (변수는 존재하지 않는다)
 *             (public static final) int a=100; // 반드시 선언과 동시에 초기값 부여 => 이후 변경불가
 *           -------------------------
 *             구현이 안된 메소드
...
class Box implements Cloneable{
	...
	// throws 문장에 뒤에 붙는 예외클래스는 => RuntimeException 관련은 없다 => 예외처리를 하고 사용한다
	@Override
...
class Food{
	...
	public Food(int fno,String name) {
		...
		// 지역 변수 우선순위
		// 변수 => 지역변수 , 매개변수 => 멤버변수
		// 변수명이 같은 경우에는 반드시 구분 => this : 클래스 자신의 객체 , super : 상속 내린 클래스의 객체
		/*
		 *     메모리
		 *     ------------------------------
		 *         ------- super ------------
...
//		Sawon s1=new Sawon(2, "박문수");
//		System.out.println(s1.toString());
		/*Student s1=new Student(1, "홍길동");
		Student s2=new Student(1, "홍길동");
		// new 를 사용하면 (새로운 메모리를 만들어서 사용)
		// 중복을 제거할때 주로 사용한다
...
//		System.out.println("s2:"+s2.hashCode());
		...
		// equals() => 재정의 => 오버라이딩을 하기 전까지 => 주소값
		System.out.println("f1="+f1);
		System.out.println("f2="+f2);
		if(f1.equals(f2)) {
			System.out.println("같은 업체다");
		}else {
			System.out.println("다른 업체다");
		}
