 ...
 *        사용자
 *         |
 *        JVM --> 운영체제 연결 => API => Write ONE (한번 소스 코딩으로 모든 운영체제에 연결할 수 있다)
 *         | --> 메모리 구조 (변수...)
 *        운영체제
 *         |
 *        하드웨어
 ...
 *        1) 알파벳이나 한글로 시작한다 (단 알파벳 사용시에는 대소문자 구분)
 *        2) 숫자 사용이 가능 (앞에 사용금지)
 *        3) 공백이 있으면 안된다
 ...
 *                   실수형의 default => 컴퓨터에서 사용하는 모든 실수 => double
 *                   float f=10.5; => 오류
 *                           ---- 컴퓨터에서 double을 인식하기 때문에
 *                            | 10.5F
 *        문자
 ...
 *     ==> 초기화
 *     byte b=100; => 127을 초과하지 않으면 된다
 *     int i=100;
 *     long 1=100L; => L은 long형값이다
 *     char c='문자1개' => 'A' => ''오류 발생 => ' '
 *     double d=10.5D; ==> D는 생략이 가능
 *     boolean b=true/falsse
