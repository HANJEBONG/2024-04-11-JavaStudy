 ...
 *                                             --------
 *              => 같은 기능끼리의 클래스를 모아서 한개의 클래스이름으로 관리할때 주로 사용
 *                 --------------------------------------
 *                 데이터 => 배열
 *                 클래스 => 추상클래스 , 인터페이스
 ...
 *               스프링에서는 상속을 권장하지 않는다
 *                        --- 제약이 존재 (상위 클래스에 맞게...)
 *                        --- 메모리가 커진다 (상위클래스 메모리 할당 => 하위 클래스 메모리 할당)
 *                        --- 처리속도가 늦다
 *                        --- 재사용을 한다 => 상위 클래스의 소스 코드를 모르는 상태에서 재정의 한다 (가독성 저하)
