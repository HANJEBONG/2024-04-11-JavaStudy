 ...
 *                   => 실전 (배포) => 가상으로 연습 
 *       => 날짜 : Date / Calendar
 *                          | 기능 부여 (요일 , 마지막날짜....) => 달력 제작
 *               ----- 기능없다 => 오라클 / MySQL에서 전송하는 날짜를 받는 경우 (데이터베이스 호환) 
 ...
 *              5. ArrayList/Vector/LinkedList 는 List 라는 인터페이스를 구현
 *                 => 메소드가 동일 (표준화)
 *                 => 주요기능
 *                    ***= add() : 추가
 *                    = remove() : 삭제
...
public class 라이브러리_1 {
	...
	public static void main(String[] args) {
		...
		// key 가 중복되면 => 덮어쓴다 => 중복을 허용하지 않는다 => Cookie
		// 값을 읽어 온다
		...
		Set<String> s=map.keySet(); // 키만 저장 => Set = 중복을 허용하면 안되어서 사용
		for(String ss:s) {
			System.out.println(ss+":"+map.get(ss));
		}
		// Set : 순서가 없다 => 가급적이면 출력시에 List (순차적이다)
		// => Map 은 클래스 정리
	}
