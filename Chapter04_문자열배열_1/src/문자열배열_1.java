 ...
 *                                     ---------------------- 메소드
 *    String 사용법
 *    String names"ASSS"; // 제한이 없다 => char[]을 클래스화
 *    --------------------
 *    -------------------- 데이터형을 제허하는 클래스 : Wrapper
 *    int =======> Integer
 *    long ======> Long
 *    byte ======> Byte
 *    double ====> Double
...
public class 문자열배열_1 {
	...
	public static void main(String[] args) {
		...
		String[] title={
			...
			// 노래 출력
			System.out.println("등록된 총 곡: "+title.length+"곡 수록");
			int rank=1;
			for(String t:title) {
			...
			int count=0;
			for(String t:title) {
				if(t.contains(fd)) { // 제목안에 단어가 포함 되어있는지 여부
			...
			// 배열이 여러개인 경우 => 인덱스 번호를 맞춘다
			// 상승 / 하락 / 유지
			for(int i=0;i<title.length;i++) { // 여러개의 배열을 동시에 제어 => 인덱스를 이용한다
				String temp="";
