 ...
 *             => 기능(메소드)
 *                toString() : 객체를 문자열화
 *                clone() : 복제(새로운 메모리 생성) => 실무 (prototype)
 *                equals() : 객체 비교
 *                finalize() : 소멸자
 *                hashCode() : 객체 주소를 변환
 *             => Object 는 가장 큰 데이터형
 ...
 *     2. java.util
 *        ---------
 *          = 클래스를 사용하기 위해서는 반드시 import 를 사용한다
 *          = 프로그램에 많이 유용하게 사용할 수 있는 클래스들의 집합
 *          = 날짜관련
 *             ***Date : 기능이 별로 없다 (오라클에서 저장된 날짜를 읽어서 저장)
 *               => 기능을 추가 : Calendar
 *          = 문자열을 분해
 ...
 *                          ML => <a> : 태그 => 사용자 정의
 *                          HTML/XML 은 오라클을 연결하는 언어가 아니다 .. (화면 출력)
 *                        | 자바는 오라클에 있는 데이터를 관리
 *        -----
