 ...
 *             클레스 전체 / 다른 클래스에 사용이 가능
 *             일반 변수와 동일
 *             => 클래스는 한개에 대한 정의
 *                => 영화 1개 / 레시피 1 개 => 저장만 여러개
 *             ** 공통변수가 사용되는 변수는 static을 이용한다
 *     생성자 : 클래스명과 동일 / 리턴형이 없다
 *            => 역할 메모리 할당할때 호출되는 메소드
 *            => 멤버변수의 초기화
 *              ------------- 서버 구동 / 화면 UI / 데이터베이스 연동
 *            => 반드시 필요하다 => 경우에 따라서 생략 => 컴파일러가 자동으로 생성자를 추가해 준다
 ...
 *         ---------------------------------------------- 통합 (Spring)
 *         
 *     *** 프로그램이 비정상 종료가 안되게 만든다. : 예외처리 => 자바 기본 문법
 *         
 */
class Movie{
	int rank;
...
public class 클래스_5 {
	...
	public static void main(String[] args) {
		...
		Movie m2=new Movie();
		m2.rank=2;
		m2.regdate="2024.05.28";
		m2.reserve="24.2%";
		m2.title="속성탈출-새로운 시대";
