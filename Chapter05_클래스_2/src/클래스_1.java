 ...
 *                --------- 자체 프록르램에서는 사용이 가능 (캡슐화)
 *                --------- 접근지정어
 *           ---------------------
 *            초기화 (변수)
 *             1. 명시적인 초기화
 *                int a=10; => 특별한 경우 아니면 사용빈도가 낮다
 *             2. 초기화 블록
 *                = 인스턴스 블록
 *                  class ClassName
 *                  {
 ...
 *                    static
 *                    {
 *                        static변수만 초기화가 가능하다 => new를 이용하지 않고 자동으로 저장이 되게 만든다
 ...
 *                              연산자
 *                              1. 클래스의 메모리의 크기를 확인
 *                              2. 메모리에 저장 => 메모리 확보 => 메모리 주소를 개체에 전송
 *                  에)
 *                     class A
 *                     {
 ...
 *                                      --------------
 *                                      
 *                             A b=new A() => new => 미리 메모리가 생성이 된다 => 접근하는 범위는 해당 주소에서만 사용이 가능
 *                                 ---
 *                                  |
 *                              --- b ---
 *                              
 ...
 *                                           -----
 *                                           ----------
 *                             => new 는 사용할때마다 미리 메모리 생성 (클래스 크기만큼) => 제한 (하드디스크 크기만큼 정용이 가능)
 *                             => new 를 이용해서 저장되는 내용
...
class Card{
	...
	int number; // 0
	String type; // null
	// 아직 저장이 안됨 ==> new를 사용해서 저장
	// 공통으로 사용되는 변수 => 정적 변수 => static
	static int width=200,height=300; // 명시적인 초기화 ==> 저장 static 은 컴파일시에 저장
	 ...
	 *                       => 메모리에 저장       return
	 *       -----------------------------------------------------------------
	 *        인스턴스 변수      new를 이용한 경우     프로그램 종료시  Heap      클래스 전체 사용이 가능
	 *                                                                 => 다른 클래스에서도 사용가능
	 *        class A
	 *        {
	 *            int aa;
	 ...
	 *               
	 *              ------
	 *             --------- a.aa(.은 메모리 주소 접근 연산자) => a주소안에 저장되어있는 aa라는 변수에 접근 
	 *       -----------------------------------------------------------------
	 *        정적 변수         컴파일시에 저장        프로그램 종료시   Method Area  클래스 전체에서 사용이 가능
