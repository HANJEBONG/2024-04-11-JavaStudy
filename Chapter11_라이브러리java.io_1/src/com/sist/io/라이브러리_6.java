...
public class 라이브러리_6 {
	...
	public static void main(String[] args) {
		...
		try {
			fis=new FileInputStream("c:\\javaDev\\back.jpeg");
			fos=new FileOutputStream("c:\\java_data\\back.jpeg");
			int i=0;
			byte[] buffer=new byte[1024];
			/*
			 *     파일 읽기
			 *     => read() => 한글자씩 읽기
			 *     => read(byte[] buffer,int s, int e) => 지정한 갯수만큼 읽기
			 */
			while((i=fis.read(buffer, 0, 1024))!=-1) {
				// i => 읽은 바이트 수
				fos.write(buffer, 0 , i); // 클라이언트에서 서버로 파일을 전송
				// => TCP : 1024 UDP : 512
				// => 프로그래바가 증가 ... (업로드 / 다운로드 사용)
			}
			System.out.println("File copy complet!!");
		} catch (Exception e) {
