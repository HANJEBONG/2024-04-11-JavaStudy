 ...
 *     java.io
 *     --------
 *     | 특징 : 데이터 이동 톨로를 만들어서 데이터를 입출력 한다
 *            ------------
 *            | 스트림 : 단점 (단방향이다)
 ...
 *           송신/수신 => 1byte => 자바에서는 각국의 언어를 사용할 수 있도록 2byte를 사용할 수 있게 제작
 *           ----------------
 *           1byte => 바이트 스트림
 ...
 *            입출력
 *             --------------------------------- 바이트 스트림 -------------
 *             => 읽기메소드 : read()  => 한글자 읽기 read(byte[])
 *             => 쓰기메소드 : write() => 한글자 입력 write(byte[]) write(int)
 *             --------------------------------------------------------- 파일 닫기 : close()
 *              FileInputStream / FileOutputStream
 ...
 *                                                       --------------
 *                FileReader / FileWriter
 *                => 크롤링후 파일에 저장 / 카페 , 블로그 .... 로그 파일 => 분석 (추천)
 *             --------------------------------------------------------
 *             
 *             ----------------------- 필터 스트림 -----------------------
 *             파일에 있는 데이터를 메모리에 한번에 이동후에 사용이 가능 => 속도가 빠르다 Buffered~
 ...
 *                             -----------  ----------
 *                                          | append => 기존의 데이터에 결합
 *                             | create => 기존의 데이터에 덮어쓴다
 *                             -----------------------------------------
 *                             생성자 new FileWriter("파일명",true)  ===> a
 *                             생성자 new FileWriter("파일명")  ========> w
 *             
 *             => 파일을 읽어서 => ArrayList에 저장후 데이터를 활용
 *                            ------------- 배열
 *                            
 *             => Properties
 *                ---------- 파일의 확장자 : .properties
 *                           구분자가 있기 때문에 사용하기 편리하다 => 사용자 정의보다는 
 *                                                          외부 라이브러리에서 주로 사용 : 보안
 *                                                          ----------
 *                                                          | 데이터베이스 관련 (MyBatis , JPA , Spring)
 *                                                                                          | => XML,Annotation
 *                                                           *** 필수적으로 사용 : Spring-boot(환경설정)
 ...
 *                                               ------------------
 *                                   | 프로그램 구동시마다 데이터 수집을 계속해야 한다
 *                                   | 데이터 수집없이 파일만 읽는다
 *                      2. 데이터 수집시 중복된 데이터 => 제거 => Set
 *                      3. 관리하는 클래스가 많은 경우
 *                         -------------------
 *                           new 메모리 누수현상 => 한개의 메모리만 사용이 가능하게 만든다
 *                                             ---------------------------- 싱글턴
 *                                                                          | Map
 *              => 컬렉션
 *                 Collection
 ...
 *        2. sql
 *        3. J2EE => 웹관련
 *        4. 외부라이브러리 : Jsoup ... mvnrepository.com
 *        5. Spring
 *        6. python 연동
 *     ----------------------------------------------------------------
 *     
