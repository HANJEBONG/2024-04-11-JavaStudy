 ...
 *            C/C++, C#, Java
 *                         ▽
 *                        One Write (모든 운영체제에서 호환이 되도록 하는 목표)
 *         = 인터프리터 방식 : 한줄씩 번역해서 처리
 *          HTML / CSS
 *         = 컴파일 / 인터프리터를 동시에 사용 => 보안 (소스검증)
 *          ---------------
 *           제임스 고슬링 => 자바언어는 절대 보안 노출되지 않는다 (얼마 지나지않아 뚫림)
 *           
 *         자바소스 (.java)
 *            |
 *           JVM ==> .class 번역
 ...
 *              2. 대소문자를 구분
 *              3. 명령문(CPU) => 끝나면 반드시 ;
 *              4. 클래스 영문외에 한글사용이 가능 -> 리눅스나 다른 운영체제에서는 한글 인식이 안될때도 있다 (권장 : 영문)
 *           --------------------------------------------------------------------------------------
 *           = 문법은 아니고 약속 사항
 *            1) 클래스명은 대문자로 시작
 *              MainClass
